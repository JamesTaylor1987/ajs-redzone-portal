  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <h1 className="text-xl font-extrabold text-ajs-dark">CRM Prospects</h1>
        <Link
          href="/admin/crm/new"
        ...
        <Link
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
            !activeStage
              ...
              : "bg-white border border-ajs-light text-ajs-muted hover:border-ajs-primary/40"
          }`}
        >
          Active
        </Link>
            ...
            {prospects.map((p) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  <span className="font-bold text-ajs-dark text-base">{p.company_name}</span>
                ...
                {firstContact[p.id] && (
                  <div className="text-sm text-ajs-muted">{firstContact[p.id]}</div>
                )}
          ...
          <div className="hidden sm:block bg-white rounded-xl border border-ajs-light overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-ajs-light">
                <tr>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Ref</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Company</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Stage</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Key contact</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Next action</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Logs</th>
                  <th className="px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wide text-ajs-dark">Owner</th>
              ...
              <tbody className="divide-y divide-ajs-light">
                {prospects.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-ajs-muted">{p.ref}</td>
                    ...
                    <td className="px-4 py-3 text-xs text-ajs-muted text-center">{actCounts[p.id] || 0}</td>

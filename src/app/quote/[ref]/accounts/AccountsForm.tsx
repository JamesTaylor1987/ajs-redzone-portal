...
import { acceptAccountsAction, type AcceptAccountsState } from "./actions";

interface Props {
  quoteRef: string;
  accountsToken: string;
  prefill: { contactName: string; contactPhone: string; contactEmail: string };
}

export function AccountsForm({ quoteRef, accountsToken, prefill }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result: AcceptAccountsState = await acceptAccountsAction(null, formData);
    if (result?.error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <input type="hidden" name="accountsToken" value={accountsToken} />
      ...
      <Section title="Invoicing">
        ...
        <div className="sm:col-span-2">
          <YesNo label="Do invoices require a PO number? *" name="requiresPONumber" />
        </div>
      </Section>
      ...
      <Section title="GDPR Consent &amp; Acceptance">
        ...
        <div className="sm:col-span-2">
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" name="gdprConsent" required className="mt-0.5 accent-ajs-primary" />
            <span className="text-sm font-semibold text-ajs-text">I agree to the above GDPR statement *</span>
          </label>
        </div>

        <Field label="Your full name *" name="signatoryName" required />
        <Field label="Your position / title *" name="signatoryPosition" required />
        <div>
          <label className="block text-xs font-bold uppercase tracking-wide text-ajs-dark mb-1">Date</label>
          <input
            type="text"
            value={today}
            readOnly
      ...
      <div className="border-t border-ajs-light pt-5">
        <p className="text-xs text-ajs-muted mb-4 leading-relaxed">
          ...
          An invoice will be issued within 24 hours, payable 100% prior to shipment. DAP delivery terms apply.
        </p>
        <button
          type="submit"
          disabled={submitting}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-xl border border-ajs-light p-5">
      <h2 className="font-bold text-sm text-ajs-dark mb-4" dangerouslySetInnerHTML={{ __html: title }} />
...
function Field({
  label, name, type = "text", required, defaultValue, hint, className = "",
}: {
  label: string; name: string; type?: string; required?: boolean;
  defaultValue?: string; hint?: string; className?: string;
}) {
  return (
...
function TextArea({ label, name, required, rows = 3 }: {
  ...
  return (
    <div>
      <label className="block text-xs font-bold uppercase tracking-wide text-ajs-dark mb-1">{label}</label>
      <textarea
        name={name} required={required} rows={rows}
...
function YesNo({ label, name, onChange }: {
  label: string; name: string; onChange?: (val: string) => void;
}) {
  return (
    <div>
      ...
      <div className="flex gap-4">
        {["yes", "no"].map((val) => (
          <label key={val} className="flex items-center gap-2 text-sm text-ajs-text cursor-pointer">
            <input
              type="radio" name={name} value={val} required
              onChange={() => onChange?.(val)} className="accent-ajs-primary"
            />
            {val === "yes" ? "Yes" : "No"}
          </label>
        ))}
